...
Item {
    ...
    readonly property int pagePaddingValue: root.tokens ? root.tokens.pagePadding : 30
    readonly property int sectionGapValue: root.tokens ? root.tokens.sectionGap : 16
    readonly property int panelRadiusValue: root.tokens ? root.tokens.panelRadius : 16
    readonly property color pagePanelBackground: root.tokens ? root.tokens.panelBackground : "#FBF8F3"
    readonly property color pageBorderColor: root.tokens ? root.tokens.borderColor : "#E1D7CA"
    ...
    ColumnLayout {
        ...
        PageHeader {
            ...
            title: "设置"
            subtitle: "集中维护 provider、密钥、运行参数与桌面数据目录，保持配置节奏和阅读工作台一致。"
        }
                ...
                ColumnLayout {
                    ...
                    Item {
                        ...
                        SectionCard {
                            ...
                            tokens: root.tokens
                            heading: "LLM 设置"
                            supportingText: "优先对齐 provider、endpoint、model、key 与 temperature，让桌面配置更像编辑台而不是维护面板。"
                        ...
                        SectionCard {
                            ...
                            tokens: root.tokens
                            heading: "Pipeline 设置"
                            supportingText: "这里保持 Python 业务逻辑不动，只把可见参数整理成更稳定的桌面表单节奏。"
                            ...
                            GridLayout {
                                ...
                                Label { text: "日报上限"; color: root.pageInkMuted }
                                SpinBox {
                                    id: maxArticlesSpin
                                    Layout.fillWidth: true
                                    from: 1
                    ...
                    SectionCard {
                        ...
                        tokens: root.tokens
                        heading: "GitHub 趋势"
                        supportingText: "把 token 与抓取预算收进同一张全宽配置卡，和上方设置保持同一节奏。"
        ...
        Rectangle {
            ...
            RowLayout {
                ...
                Label {
                    Layout.fillWidth: true
                    text: settingsFacade && settingsFacade.stale ? "当前有未保存的配置改动。" : "当前配置已与 facade 同步。"
                    color: root.pageInkMuted
                    font.family: root.pageSansFamily
                    font.pixelSize: 13
